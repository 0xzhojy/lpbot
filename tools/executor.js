...
import { getWalletBalances, swapToken, getOnChainTokenBalanceAndPrice } from "./wallet.js";
...
import { notifyDeploy, notifyClose, notifySwap, notifyChartIndicators } from "../telegram.js";
...
function normalizeConfigValue(key, value) {
  ...
  // These keys accept null to disable the feature
  const nullableNumberKeys = new Set(["takeProfitUsd", "maxTvl", "minMcap", "maxMcap", "athFilterPct", "minTokenAgeHours", "maxTokenAgeHours"]);
  if (nullableNumberKeys.has(key) && (value === null || value === "" || value === "null" || value === "none")) return null;
...
const toolMap = {
  ...
  update_config: ({ changes, reason = "" }) => {
    ...
    const CONFIG_MAP = {
      ...
      takeProfitUsd: ["management", "takeProfitUsd"],
  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        const pairLabel = result.pool_name || args.pool_name || args.pool_address?.slice(0, 8);
        notifyDeploy({ pair: pairLabel, amountSol: args.amount_y ?? args.amount_sol ?? 0, position: result.position, tx: result.txs?.[0] ?? result.tx, priceRange: result.price_range, rangeCoverage: result.range_coverage, binStep: result.bin_step, baseFee: result.base_fee }).catch(() => { });
        if (result.base_mint) {
          notifyChartIndicators({ pair: pairLabel, mint: result.base_mint }).catch(() => { });
        }
